...
public class DecodeMessage {
  ...
  private static void decodeMessage(String signedData) {
    ...
    try {
      log.info(signedData);
      log.info("解密 start " + System.currentTimeMillis());
      ...
      BigInteger pubKey =
          Sign.signedMessageToKey(TransactionEncoder.encode(rawTransaction), signatureData);
  ...
  /**
   * 可以看到交易数据本身是没有加密的，是可以直接获取到。 v r s是用私钥加密的数据，利用v r s加上交易数据可以得到私钥对应的公钥及地址。
   * 所以RawTransaction里是没有fromAddress的参数的。 解密出的地址就是发出交易的地址。这样一来完成了验证。
   */
