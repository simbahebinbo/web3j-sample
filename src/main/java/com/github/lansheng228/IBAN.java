...
public class IBAN {
  ...
  /**
   * 根据官方支持的IBAN规则生成二维码 目前支持的有imtoken kcash 参考url
   * https://github.com/ethereum/web3.js/blob/develop/lib/web3/iban.js 可以防止地址错误（有两位校验和）
   */
  ...
  private static boolean validateIBAN(String iban) {
    int len = iban.length();
    if (len < 4 || !iban.matches("[0-9A-Z]+")) return false;

    iban = iban.substring(4) + iban.substring(0, 4);

    StringBuilder sb = new StringBuilder();
    for (int i = 0; i < len; i++) sb.append(Character.digit(iban.charAt(i), 36));
