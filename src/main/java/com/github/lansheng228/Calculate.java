...
public class Calculate {
  public static void main(String[] args) {
    log.info(
        calculateContractAddress("0x6c0f49aF552F2326DD851b68832730CB7b6C0DaF".toLowerCase(), 294));
    ...
    log.info(caculateTransactionHash(signedData));
  }

  /** 发布前 计算合约地址 */
  private static String calculateContractAddress(String address, long nonce) {
    // 样例
    // https://ropsten.etherscan.io/tx/0x728a95b02beec3de9fb09ede00ca8ca6939bad2ad26c702a8392074dc04844c7
    byte[] addressAsBytes = Numeric.hexStringToByteArray(address);

    byte[] calculatedAddressAsBytes =
        Hash.sha3(
            RlpEncoder.encode(
                new RlpList(RlpString.create(addressAsBytes), RlpString.create((nonce)))));

    calculatedAddressAsBytes =
        Arrays.copyOfRange(calculatedAddressAsBytes, 12, calculatedAddressAsBytes.length);
  ...
  private static String caculateTransactionHash(String signedData) {
    // 样例
    // https://ropsten.etherscan.io/tx/0xfd8acd10d72127f29f0a01d8bcaf0165665b5598781fe01ca4bceaa6ab9f2cb0
