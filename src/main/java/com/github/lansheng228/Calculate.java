package com.github.lansheng228;

import lombok.extern.slf4j.Slf4j;
import org.web3j.crypto.Hash;
import org.web3j.rlp.RlpEncoder;
import org.web3j.rlp.RlpList;
import org.web3j.rlp.RlpString;
import org.web3j.utils.Numeric;

import java.util.Arrays;

@Slf4j
public class Calculate {

    public static void main(String[] args) {
        calculateContractAddress("0x6c0f49aF552F2326DD851b68832730CB7b6C0DaF".toLowerCase(), 294);

        String signedData =
                "";
        calculateTransactionHash(signedData);
    }

    /**
     * 发布前 计算合约地址
     * <p>
     * 样例  https://ropsten.etherscan.io/tx/0x728a95b02beec3de9fb09ede00ca8ca6939bad2ad26c702a8392074dc04844c7
     */
    private static String calculateContractAddress(String address, long nonce) {
        byte[] addressAsBytes = Numeric.hexStringToByteArray(address);
        byte[] calculatedAddressAsBytes = Hash.sha3(RlpEncoder.encode(new RlpList(RlpString.create(addressAsBytes), RlpString.create(nonce))));
        calculatedAddressAsBytes = Arrays.copyOfRange(calculatedAddressAsBytes, 12, calculatedAddressAsBytes.length);
        String calculatedAddressAsHex = Numeric.toHexString(calculatedAddressAsBytes);
        log.info("calculatedAddressAsHex: " + calculatedAddressAsHex);

        return calculatedAddressAsHex;
    }

    /**
     * 提交前 计算交易hash
     * <p>
     * 样例  https://ropsten.etherscan.io/tx/0xfd8acd10d72127f29f0a01d8bcaf0165665b5598781fe01ca4bceaa6ab9f2cb0
     */
    private static String calculateTransactionHash(String signedData) {
        String txHash = Hash.sha3(signedData);
        log.info("txHash: " + txHash);

        return txHash;
    }
}
